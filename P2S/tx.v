...
//    file: tx.v
//    Function: parallel to serial shifter 
//   
//  
/*
      Description: With this design the diesize cost is not an issue. We more
         ...
         This design include improving:
           1) Have 2 parallel to serial converter. This will help to improve the
              speed of the chip. While converter1 is continue shifting to serial  
              port converter can down load next transmit data from controller. At
              the end of bit transmit of converter 1 the controller will automatic
              swith to converter for continue shifting out data. The data shift 
              to serial port will continue not halt by waiting to load the new data
              from controller.
           2) Add a controller circuit. This controller will always check the 
              converter1 and converter2 available or not. If any one of them 
              available an signal TX_READY_LD will send to controller to transmit 
              next byte.
                This controller must make sure to check both converter1 and 
              converter2 send last byte that they are receive to serial port. If 
              they are down load the transmit data and did not complete to send to 
              serial port, a memory flag will set to let controller to wait them 
              finish to transmit to serial port.  

           3) An TX_READY_LD controller signal will send to controller when any 
              converter is available to load the new transmit byte. By default 
              converter1 will get higher priority.


           4) Add the last byte control signal. THe last byte will received from 
              controller and wait until the last bit of byte is transfered to serial 
              port then it will be send to bit stuff block
   ...
   __|    |____|    |____|    |____|    |____|    |____|    |
                  ...
                  ________  ________ 
 TDO ------------<________><________>--------------   


                      ________  ________ 
 TX_DATA      -------<________><________>--------------   
                         |
                         |
                         |
                         |
      ...
      - CONTROLLER WILL SEND SYNC signal and P2S will generate sync serial signal
      - CONTROLLER WILL SEND TRANSMIT DATA 11 22 33 44
      - CHECK P2S will transmit data 11 22 33 44 on TDO with timing below
      - CHECK TX_READY_LD will be active when either shifter1 or shifter2 
        finish to transmit the last bit of byte.
      - CHECK TX will send the sync signal to SERIAL BUS
      - VERIFY shift1_done or shift2_done will active when they finish to transmit a byte
      - VERIFY RCS.
      - VERIFY THE T_lastbit transmit to Serial port.
...
input reset;			//system reset; global; active high
...
output TCS; // means that one of the shifter is shifting 
output TX_READY_LD;
wire TCS;
wire T_lastbit1; 
reg  T_lastbit2;
